...
If @error <> 1 Then
GUICtrlSetData($iEdit2, "Подождите!" & @CRLF & "Создается список файлов" & @CRLF, 1)
FileWriteLine ($iLogFile, "Подождите!" & @CRLF & "Создается список файлов" & @CRLF)
...
$iFolderList = FileSelectFolder("", "")
If @error <> 1 Then
GUICtrlSetData($iEdit1, "Подождите!" & @CRLF & "Создается список файлов" & @CRLF, 1)
FileWriteLine ($iLogFile, "Подождите!" & @CRLF & "Создается список файлов" & @CRLF)
$iFile = FileOpen(@TempDir & "\ogg_tools\temp.bat", 10)
FileWriteLine ($iFile, "chcp 65001")
FileWriteLine ($iFile, "@echo off")
...
FileClose ($iLIPFile)
FileClose ($iFile)
FileWriteLine ($iLogFile, "Выполнено!" & @CRLF & "Файл " & $iName & ".lip" & " сохранен!" & @CRLF)
...
FileWriteLine ($iLogFile, "Выполнено!" & @CRLF & "Файл " & $iName & ".ogg" & " сохранен!" & @CRLF)
Else
GUICtrlSetData ($iEdit1, 'Ошибка! Файл ' & $iName & $iExp & ' не является аудио архивом из игры "Of orc and human"' & @CRLF, 1)
FileWriteLine ($iLogFile, 'Ошибка! Файл ' & $iName & $iExp & ' не является аудио архивом из игры "Of orc and human"' & @CRLF)
EndIf
EndFunc
...
ShellExecuteWait (@ScriptDir & "\gzip.exe", ' -d ' & @TempDir & '\ogg_tools\' &  $iName & '.gz', '', "open", @SW_HIDE)
...
;FileWrite ($iNewFile, "0x0000000F000000130000000F");ЗДЕСЬ ОШИБКА!!!!!!
...
ShellExecuteWait (@ScriptDir & "\gzip.exe", ' -1 ' & @ScriptDir & '\OUT\' & $iName, '', "open", @SW_HIDE)
FileMove (@ScriptDir & '\OUT\' & $iName & '.gz', @ScriptDir & '\OUT\' & $iName & '.pgz')
GUICtrlSetData($iEdit2, "Выполнено!" & @CRLF & "Файл " & $iName & " сохранен!" & @CRLF, 1)
FileWriteLine ($iLogFile, "Выполнено!" & @CRLF & "Файл " & $iName & " сохранен!" & @CRLF)
Else
GUICtrlSetData ($iEdit2, 'Ошибка! Отсутствует файл ' & $iName & ".lip!" & @CRLF, 1)
FileWriteLine ($iLogFile, 'Ошибка! Отсутствует файл ' & $iName & ".lip!" & @CRLF)
EndIf
;Else
;GUICtrlSetData ($iEdit2, 'Ошибка! Файл ' & $iName & $iExp & ' не является OGG Vorbis файлом' & @CRLF, 1)
;FileWriteLine ($iLogFile, 'Ошибка! Файл ' & $iName & $iExp & ' не является OGG Vorbis файлом' & @CRLF)
EndIf
FileClose ($iOGGFile)
EndFunc
